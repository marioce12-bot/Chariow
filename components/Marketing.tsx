...
const faqs = [
  ...
  ["Y a-t-il une limite d'utilisation de l'IA ?", "Non, l'accès à l'assistant IA est illimité tant que ton abonnement est actif."],
...
function AnimatedCounter({ end, duration = 1600, className }: { end: number; duration?: number; className?: string }) {
...
// Bandeau défilant : captures d'écrans de l'app, défilement continu de droite à gauche.
const appScreens = [
...
export function Marketing({ proofImages = [] }: { proofImages?: ProofImage[] }) {
  ...
  return <main>
     ...
     <section className="section screens-section" style={{paddingTop:0}}><div className="container"><div className="section-head"><div><span className="eyebrow">Dans ton espace</span><h2>Tes ventes et tes pubs,<br/>au même endroit.</h2></div><p>Un aperçu de ce que tu retrouves une fois ta boutique et tes comptes publicitaires connectés.</p></div></div><div className="screens-marquee"><div className="screens-track">{[0,1].map((copy) => <div className="screens-group" key={copy} aria-hidden={copy===1}>{appScreens.map(([src, alt]) => <Image key={`${copy}-${src}`} className="screens-image" src={src} alt={alt} width={760} height={524}/>)}</div>)}</div></div></section>
