  ...
  return (
    <>
      ...
      {!accounts.length ? <div className="empty-state"><BarChart3 size={24} /><strong>Aucun compte Meta Ads connecté</strong><span>Autorise Vendeo à lire tes campagnes, ensembles de publicités et publicités.</span><button className="btn btn-dark" onClick={connect}>Connecter Meta Ads</button></div> : <>
        ...
        {performance ? <><div className="vendeo-kpi-grid meta-kpis"><div className="vendeo-kpi"><small>Dépenses Meta</small><strong>{formatMoney(performance.overview.spend, performance.currency)}</strong></div><div className="vendeo-kpi"><small>CA Chariow réel</small><strong>{formatMoney(performance.overview.chariowRevenue, performance.currency)}</strong></div><div className="vendeo-kpi"><small>CPA Meta</small><strong>{performance.overview.cpa === null ? "n/d" : formatMoney(performance.overview.cpa, performance.currency)}</strong></div><div className="vendeo-kpi"><small>CAC par vente Chariow</small><strong>{performance.overview.cac === null ? "n/d" : formatMoney(performance.overview.cac, performance.currency)}</strong></div><div className="vendeo-kpi"><small>ROAS Meta rapporté</small><strong>{performance.overview.metaRoas === null ? "n/d" : `${performance.overview.metaRoas.toFixed(2)}x`}</strong></div><div className="vendeo-kpi"><small>ROAS réel attribué</small><strong>{performance.overview.realRoas === null ? "n/d" : `${performance.overview.realRoas.toFixed(2)}x`}</strong></div></div><div className="meta-attribution-note">{performance.overview.attributionCoverage ? `${performance.overview.attributionCoverage} vente(s) reliée(s) par attribution interne.` : "Aucune vente Chariow n’est encore reliée à une publicité. Le ROAS réel reste indisponible jusqu’à la mise en place des UTMs/click IDs."}</div><section className="app-card meta-campaigns"><div className="card-head"><div><span className="eyebrow">Analyse média</span><h2>Campagnes qui gagnent ou brûlent du cash</h2></div><Activity size={18} color="#103ef8" /></div><div className="meta-table"><div className="meta-table-head"><span>Campagne</span><span>Dépenses</span><span>CPA Meta</span><span>ROAS Meta</span><span>Statut</span></div>{performance.performances.map((campaign) => <div className="meta-table-row" key={campaign.id}><strong>{campaign.name}</strong><span>{formatMoney(campaign.spend, performance.currency)}</span><span>{campaign.cpa === null ? "n/d" : formatMoney(campaign.cpa, performance.currency)}</span><span>{campaign.roas === null ? "n/d" : `${campaign.roas.toFixed(2)}x`}</span><span className={`meta-status ${campaign.status}`}>{campaign.status === "profitable" ? "Rentable" : campaign.status === "loss" ? "À corriger" : "Sans signal"}</span></div>)}</div>{!performance.performances.length && <p className="profit-help">Aucune campagne synchronisée. Lance une synchronisation Meta Ads.</p>}</section></> : <div className="empty-state">Synchronise ton compte pour afficher les performances.</div>}
      </>}
    </>
  );
}
