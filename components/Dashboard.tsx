...
function MetaAdsView({ products, onPromoteProduct, campaignRefreshKey }: { products: ProductData[]; onPromoteProduct: (product: ProductData) => void; campaignRefreshKey: number }) {
  ...
  const [resources, setResources] = useState<{ pages: Array<{ id: string; name: string }>; pixels: Array<{ id: string; name: string }> } | null>(null);
  ...
  async function load() {
    ...
    if (nextAccounts[0]) {
      ...
      const resourceResponse = await fetch(`/api/integrations/meta/resources?account_id=${encodeURIComponent(nextAccounts[0].id)}`);
      if (resourceResponse.ok) { const resourceData = await resourceResponse.json(); setResources(resourceData); setAccountRestricted(Boolean(resourceData.account?.restricted)); }
    }
    setLoading(false);
  }
  ...
  return (
    <>
      <div className="page-top"><div><span className="eyebrow">Acquisition rentable</span><h1>Meta Ads</h1><p>Relie tes dépenses publicitaires aux ventes Chariow pour distinguer croissance et perte.</p></div>{accounts.length === 0 ? <button className="btn btn-dark" onClick={connect}><Plus size={15} /> Connecter Meta Ads</button> : <span className="status-positive meta-connected-badge"><CheckCircle2 size={14} /> Meta Ads connectée</span>}</div>
       ...
       <section className="app-card" style={{ marginBottom: 18 }}><div className="card-head"><div><span className="eyebrow">Nouveau</span><h2>Promouvoir un produit</h2><p>Choisis un produit Chariow pour préparer une campagne Meta sans quitter Vendeo.</p></div><Rocket size={19} /></div>{accountRestricted ? <div className="meta-account-warning" role="alert"><AlertTriangle size={18} /><div><strong>Ton compte publicitaire Meta est restreint</strong><p>Vendeo ne peut pas lancer de publicité avec ce compte tant que Meta n’a pas levé la restriction.</p><a href="https://www.facebook.com/accountquality" target="_blank" rel="noreferrer" className="btn btn-ghost">Vérifier dans Meta</a></div></div> : null}{accounts.length && resources && !resources.pages.length ? <div className="meta-conversion-info">Ajoute une page Facebook à ton Business Manager pour pouvoir créer une publicité. Le paiement et les moyens de paiement restent gérés directement par Meta.</div> : null}{products.length ? <div style={{ display: "grid", gap: 10, marginTop: 15 }}>{products.map((product) => <div className="store-row" key={product.id}><div className="store-logo">{product.image ? <img src={product.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 8 }} /> : <Package size={18} />}</div><div className="store-info"><strong>{product.name}</strong><span>{formatProductPrice(product)}</span></div><button className="btn btn-dark" type="button" onClick={() => onPromoteProduct(product)}>Promouvoir</button></div>)}</div> : <EmptyState title="Aucun produit disponible" text="Connecte et synchronise une boutique Chariow pour promouvoir un produit." />}</section>
       <CampaignList campaigns={campaigns} products={products} accounts={accounts} pages={resources?.pages ?? []} onConnect={connect} onRefresh={() => void load()} />
...
function CampaignList({ campaigns, products, accounts, pages, onConnect, onRefresh }: { campaigns: AdCampaign[]; products: ProductData[]; accounts: Array<{ id: string; name: string | null; currency: string; account_status?: number | null }>; pages: Array<{ id: string; name: string }>; onConnect: () => void; onRefresh: () => void }) {
  ...
  async function launch(campaign: AdCampaign) {
    ...
    const page = pages.find((item) => item.id === (selectedPage[campaign.id] ?? pages[0]?.id));
