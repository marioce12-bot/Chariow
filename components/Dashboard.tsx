...
function RealTrendChart({ sales, spend, currency }: { sales: unknown[]; spend: number; currency: string }) {
  ...
  const values = days.map((day) => rows.filter((row) => String(row.created_at ?? row.createdAt ?? row.occurred_at).slice(0, 10) === day).reduce((sum, row) => sum + Number((row.amount as Record<string, unknown>)?.value ?? row.amount ?? 0), 0));
