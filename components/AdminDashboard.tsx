...
export function AdminDashboard() {
  ...
  if (error || !data) return <main className="admin-shell"><div className="admin-error"><ShieldCheck size={25} /><h1>Accès administrateur</h1><p>{error || "Session administrateur introuvable."}</p><a className="admin-link" href="/admin/login">Se connecter</a></div></main>;
...
function UsersSection({ data }: { data: AdminData }) { return <section className="admin-card"><div className="admin-card-head"><h3>Utilisateurs inscrits</h3><Users size={17} /></div><AdminTable headers={["Nom", "Email", "Inscription", "Identifiant"]}>{data.users.map((user) => <div className="admin-row" key={user.id}><strong>{user.full_name || "Sans nom"}</strong><span>{user.email || "—"}</span><span>{new Date(user.created_at).toLocaleDateString("fr-FR")}</span><code>{user.id.slice(0, 8)}…</code></div>)}</AdminTable></section>; }
