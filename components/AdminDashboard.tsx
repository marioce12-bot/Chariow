...
function SubscriptionsSection({ data }: { data: AdminData }) { return <section className="admin-card"><div className="admin-card-head"><h3>Suivi des abonnements</h3><CreditCard size={17} /></div><AdminTable headers={["Utilisateur", "Statut", "Usage IA"]}>{data.subscriptions.map((item) => <div className="admin-row" key={item.id}><code>{item.user_id.slice(0, 8)}…</code><span className={`admin-status ${item.status}`}>{item.status}</span><span>Illimité</span></div>)}</AdminTable></section>; }
