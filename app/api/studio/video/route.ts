...
export async function POST(request: Request) {
  ...
  const reservation = await admin.rpc("reserve_credits", { target_user_id: user.id, amount: cost, operation_name: "studio_video", model_name: "fal-video", provider_amount: Math.round(cost / 1.5), request_id: requestId });
  ...
  try {
    const jobId = await createFalVideo(prompt, { duration, resolution, aspectRatio, referenceUrl, referenceMode });
