...
export async function GET(request: Request) {
  ...
  const { data: attributedSales } = store ? await supabase.from("meta_attributions").select("chariow_sale_id,meta_campaign_id,attribution_confidence,attributed_revenue").eq("user_id", user.id).eq("store_id", store.id).gte("attributed_at", `${from}T00:00:00.000Z`).lte("attributed_at", `${to}T23:59:59.999Z`) : { data: [] };
  const attributedRevenue = (attributedSales ?? []).reduce((total: number, item: { attributed_revenue?: number | string | null }) => total + (Number(item.attributed_revenue ?? 0) || 0), 0);
