...
export async function POST(request: Request) {
  ...
  const event = JSON.parse(rawBody) as { event?: string; data?: { id?: string; status?: string; amount?: string | number; currency?: string; metadata?: { userId?: string; plan?: "starter" | "pro" } } };
  if (event.event !== "transaction.success") return NextResponse.json({ received: true });
  const data = event.data;
  const userId = data?.metadata?.userId;
  const plan = data?.metadata?.plan;
  if (!data?.id || !userId || !plan) return NextResponse.json({ error: "Métadonnées de paiement manquantes" }, { status: 400 });
  const amount = Number(data.amount);
  if (data.status !== "SUCCESS" || amount !== (plan === "pro" ? 9000 : 5000) || data.currency !== "XOF") return NextResponse.json({ error: "Transaction SasPay non vérifiée" }, { status: 400 });
  ...
  const { error } = await admin.from("subscriptions").update({ plan, messages_limit: plan === "pro" ? 1200 : 400, status: "active", trial_active: false, messages_used_this_month: 0, current_period_start: now.toISOString().slice(0, 10), current_period_end: periodEnd, updated_at: now.toISOString() }).eq("user_id", userId);
