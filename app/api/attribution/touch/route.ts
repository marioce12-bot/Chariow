...
export async function POST(request: Request) {
  ...
  if (visitorId === undefined || Object.values(fields).some((value) => value === undefined)) return NextResponse.json({ error: "Données d’attribution invalides" }, { status: 400 });
  if (!visitorId) return NextResponse.json({ error: "visitor_id est requis" }, { status: 400 });
  const ip = clientIp(request);
  if (limited(ip)) return NextResponse.json({ error: "Trop de requêtes" }, { status: 429 });
  ...
  const { data: existing } = await supabase.from("attribution_touches").select("captured_at,utm_source,utm_medium,expires_at").is("store_id", null).eq("visitor_id", visitorId).order("captured_at", { ascending: false }).limit(1).maybeSingle();
  if (!shouldReplaceTouch(existing, incoming)) return NextResponse.json({ received: true, preserved: true });
  const { error } = await supabase.from("attribution_touches").insert({ user_id: null, store_id: null, visitor_id: visitorId, ...fields, captured_at: incoming.captured_at, expires_at: incoming.expires_at });
