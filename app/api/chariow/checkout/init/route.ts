...
export async function POST(request: Request) {
  ...
  try {
    const data = await createChariowCheckout({ productId, email, firstName, lastName, phone: { number: phoneNumber, countryCode }, redirectUrl: typeof body?.redirect_url === "string" ? body.redirect_url.slice(0, 2048) : undefined, customerIp: request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || undefined, customMetadata: metadata, apiKey: decryptSecret(store.chariow_api_key_encrypted) });
