...
export async function GET() {
  ...
  const { data, error } = await supabase.from("platform_payouts").select("*").order("created_at", { ascending: false }).limit(100);
...
export async function POST(request: Request) {
  ...
  const { data: payout, error: insertError } = await supabase.from("platform_payouts").insert({ requested_by: admin.id, idempotency_key: idempotencyKey, amount_xof: amount, net_amount_xof: amount, recipient_name: name, recipient_phone: phone, recipient_country: country, recipient_method: method, status: "processing" }).select().single();
  ...
  try {
    ...
    const { data: updated, error } = await supabase.from("platform_payouts").update({ provider_payout_id: result.id ?? null, status: result.status?.toLowerCase() === "completed" ? "completed" : "processing", metadata: result }).eq("id", payout.id).select().single();
    if (error) throw error;
    await writeAdminAudit(supabase, admin.id, "platform_payout_initialized", "platform_payout", payout.id, { amount_xof: amount, provider_payout_id: result.id });
    return NextResponse.json({ payout: updated }, { status: 201 });
  } catch (error) {
    await supabase.from("platform_payouts").update({ status: "failed", failure_reason: error instanceof Error ? error.message : "Erreur SasPay" }).eq("id", payout.id);
